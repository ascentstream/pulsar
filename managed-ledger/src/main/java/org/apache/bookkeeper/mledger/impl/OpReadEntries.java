...
class OpReadEntries implements ReadEntriesCallback {
    ...
    private void internalReadFromLedger(ReadHandle readHandle) {
        ...
        if (firstEntry > lastEntryInLedger) {
            if (log.isDebugEnabled()) {
                log.debug("[{}] No more messages to read from ledger={} lastEntry={} readEntry={}", ledger.getName(),
                        readHandle.getId(), lastEntryInLedger, firstEntry);
            }
            LedgerHandle currentLedger = ledger.currentLedger;
        ...
        if (log.isDebugEnabled()) {
            log.debug("[{}] Reading entries from ledger={} first={} last={}", ledger.getName(), readHandle.getId(),
    ...
    public void readEntriesComplete(List<Entry> returnedEntries, Object ctx) {
        try {
            ...
            log.error("[{}] Fallback to readEntriesFailed for exception in readEntriesComplete. op={}",
                    ledger.getName(), this, throwable);
    ...
    private void internalReadEntriesComplete(List<Entry> returnedEntries) {
        if (returnedEntries.isEmpty()) {
            ...
            log.warn("[{}] Read no entries unexpectedly; completing read. op={}", ledger.getName(), this);
            completeSuccessfully();
            return;
        }

        if (log.isDebugEnabled()) {
            log.debug("[{}] Read entries succeeded batchSize={} cumulativeSize={} requestedCount={}",
                    ledger.getName(), returnedEntries.size(), entries.size(), count);
        }
    ...
    private void internalReadEntriesFailed(ManagedLedgerException exception) {
        ...
        if (ledger.getConfig().isAutoSkipNonRecoverableData()
            ...
            if (skippedTo == null) {
                log.warn("[{}] Read failed; cannot find a position to skip to. readPosition={} exMsg={}",
                        ledger.getName(), readPosition, exception.getMessage());
        ...
        log.warn("[{}] Read failed from ledger. readPosition={}", ledger.getName(), readPosition, exception);
        completeExceptionally(exception);
    }
    ...
    private void executeOnLedgerExecutor(Runnable task) {
        try {
            ...
            log.warn("[{}] ML executor rejected task; running inline. exMsg={}", ledger.getName(), e.getMessage());
            task.run();
        }
    }
